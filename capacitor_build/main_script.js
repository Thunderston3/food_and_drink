...
const Http = window.Capacitor?.Plugins?.Http || window.Capacitor?.Http;
...
searchForBtnGroup.innerHTML = `
    ...
    <button type="button" id="searchForCoffee" class="sort-btn" style="">Coffee</button>
`;
...
const searchForCoffeeBtn = document.getElementById('searchForCoffee');
let searchForValue = 'RESTAURANTS';
if (searchForRestaurantsBtn && searchForBarsBtn && searchForCoffeeBtn) {
    searchForRestaurantsBtn.addEventListener('click', () => {
        ...
        searchForCoffeeBtn.classList.remove('active');
    });
    ...
    searchForCoffeeBtn.addEventListener('click', () => {
        searchForValue = 'COFFEE';
        searchForCoffeeBtn.classList.add('active');
        searchForRestaurantsBtn.classList.remove('active');
        searchForBarsBtn.classList.remove('active');
    });
}
...
async function performSearch() {
    ...
    try {
        ...
        } else if (searchForValue === 'COFFEE') {
            categoryId = '63be6904847c3692a84b9bb6';
        }
        
        console.log('Calling searchPlaces')
...
async function geocodeLocation(locationString) {
    try {
        const url = config.GOOGLE_MAPS_GEOCODE_URL + `?address=${encodeURIComponent(locationString)}&key=${config.GOOGLE_MAPS_API_KEY}`;
        console.log('Geocode request URL (Google Maps):', url);

        const options = {
            url: url,
            headers: {
                'Authorization': 'Bearer ' + config.GOOGLE_MAPS_API_KEY,
        ...
        if (!first.geometry || !first.geometry.location) {
            throw new Error('No geocode found for location');
        }
        
        console.log('Geocode lat & long returned: ' + first.geometry.location.lat + ', ' + first.geometry.location.lng);
...
async function searchPlaces(latitude, longitude, query = '', radius = 3219, sort = 'DISTANCE', categoryId = '4d4b7105d754a06374d81259') {
    try {
        ...
        const url = config.FOURSQUARE_BASE_URL + '/search?' + new URLSearchParams(params);
        ...
        console.error('Search API error:', error, ' - ', error.message);
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (config.FOURSQUARE_API_KEY === 'YOUR_FOURSQUARE_API_KEY') {
